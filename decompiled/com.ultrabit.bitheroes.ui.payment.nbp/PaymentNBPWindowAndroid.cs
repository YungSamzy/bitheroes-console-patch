...
public class PaymentNBPWindowAndroid : PaymentNBPWindowHandler
{
		...
		else
		{
			try
			{
				...
				case PaymentUtilityBehaviour.ValidateAndroidReceiptResponse.CODE.SUCCESS:
					...
					KongregateAnalytics.trackPaymentSuccess(response.receiptObject.jsonData, KongregateAnalytics.getPaymentGameFields(_nbpWindow.paymentRef.getPaymentStatType, _nbpWindow.paymentRef.credits), response.receiptObject.signature);
