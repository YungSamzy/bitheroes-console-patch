...
namespace com.ultrabit.bitheroes.ui.lists.itemcraftreforgelist;

public class ItemCraftReforgeList : OSA<BaseParamsWithPrefab, MyListItemViewsHolder>
{
	...
	[HideInInspector]
	public UnityEvent REFORGE = new UnityEvent();

	public SimpleDataHelper<ReforgeItemModel> Data { get; private set; }

	public void StartList()
	{
		if (Data == null)
		{
			Data = new SimpleDataHelper<ReforgeItemModel>(this);
			base.Start();
		}
	}
	...
	{
		ReforgeItemModel model = Data[newOrRecycled.ItemIndex];
		newOrRecycled.craftBtn.GetComponentInChildren<TextMeshProUGUI>().text = Language.GetString("ui_reforge");
		...
		itemIcon.SetItemData(new ItemData(model.resultRef, -1));
		itemIcon.SetItemActionType(0);
		foreach (ItemData requiredItem in model.reforgeRef.requiredItems)
		{
		...
		bool flag = model.reforgeRef.RequirementsMet();
		...
		EquipmentRef equipmentRef = model.resultRef as EquipmentRef;
		EquipmentRef equipmentRef2 = model.itemRef as EquipmentRef;
	...
	private void DoItemReforge(ReforgeItemModel model)
	{
		GameData.instance.audioManager.PlaySoundLink("buttonclick");
		GameData.instance.windowGenerator.NewPromptMessageWindow(Language.GetString("ui_confirm"), Language.GetString("ui_reforge_confirm", new string[1] { model.itemRef.coloredName }), null, null, delegate
		{
			OnConfirm(model);
		});
	}

	private void OnConfirm(ReforgeItemModel model)
	{
		GameData.instance.main.ShowLoading();
		MerchantDALC.instance.AddEventListener(CustomSFSXEvent.getEvent(MerchantDALC.ITEM_REFORGE), OnItemReforge);
		MerchantDALC.instance.doItemReforge(model.itemRef, model.resultRef);
		_itemRef = model.itemRef;
	}

	private void OnItemReforge(BaseEvent e)
	{
		DALCEvent obj = e as DALCEvent;
		GameData.instance.main.HideLoading();
		MerchantDALC.instance.RemoveEventListener(CustomSFSXEvent.getEvent(MerchantDALC.ITEM_REFORGE), OnItemReforge);
		...
		GameData.instance.PROJECT.character.armory.UpdateArmorySlotWithItemRef(list, _itemRef);
		KongregateAnalytics.checkEconomyTransaction("Craft Reforge", list2, list, sfsob, "Craft", 1);
	...
	public void AddItemsAt(int index, IList<ReforgeItemModel> items)
	{
	...
	public void SetItems(IList<ReforgeItemModel> items)
	{
	...
	{
		ReforgeItemModel[] newItems = new ReforgeItemModel[count];
	...
	private void OnDataRetrieved(ReforgeItemModel[] newItems)
	{
