...
public class RuneTile : ItemIcon
{
	...
	private RunesWindow _runesWindow;
	...
	public RunesWindow runesWindow => _runesWindow;

	public RuneRef runeRef => _runeRef;

	public int runeType => _runeType;

	public void LoadDetails(RunesWindow runesWindow, int slot, Runes runes, RuneRef runeRef = null, bool changeable = false, int runeType = -1)
	{
	...
	private void DoSelected()
	{
		if (_changeable)
		{
			...
			{
				GameData.instance.windowGenerator.NewRuneOptionsWindow(_slot, this);
			}
			else
			{
				ShowUsableRunes();
	...
	public void ShowUsableRunes(object parent = null)
	{
		List<ItemData> availableRunes = GameData.instance.PROJECT.character.getAvailableRunes(_slot);
		if (availableRunes.Count <= 0)
		{
			GameData.instance.windowGenerator.NewConfirmMessageWindow(Language.GetString("error_name"), Language.GetString("error_rune_insert_unavailable"));
			return;
		}
		for (int i = 0; i < availableRunes.Count; i++)
		{
			(availableRunes[i].itemRef as RuneRef).runeTile = this;
			(availableRunes[i].itemRef as RuneRef).runeAction = 17;
		}
		GameData.instance.windowGenerator.NewItemListWindow(availableRunes, compare: false, added: false, Util.FirstCharToUpper(RuneRef.getRuneTypeName(runeType)), large: false, forceNonEquipment: true, select: true, null, parent as GameObject);
	}

	public void ShowChangeableRunes(object parent = null)
	{
		List<ItemData> changeableRunes = GameData.instance.PROJECT.character.runes.getChangeableRunes(_slot);
		if (changeableRunes.Count <= 0)
		{
			GameData.instance.windowGenerator.NewConfirmMessageWindow(Language.GetString("error_name"), Language.GetString("error_rune_change_unavailable"));
			return;
		}
		for (int i = 0; i < changeableRunes.Count; i++)
		{
			(changeableRunes[i].itemRef as RuneRef).runeTile = this;
			(changeableRunes[i].itemRef as RuneRef).runeAction = 19;
		}
		GameData.instance.windowGenerator.NewItemListWindow(changeableRunes, compare: false, added: false, Util.FirstCharToUpper(RuneRef.getRuneTypeName(runeType)), large: false, forceNonEquipment: true, select: true, null, parent as GameObject);
	}

	public void ShowExchangeableRunes(object parent = null)
	{
		List<ItemData> changeableRunes = GameData.instance.PROJECT.character.runes.getChangeableRunes(_slot);
		if (changeableRunes.Count <= 0)
		{
		...
		{
			(changeableRunes[i].itemRef as RuneRef).runeTile = this;
			(changeableRunes[i].itemRef as RuneRef).runeAction = 18;
		}
		GameData.instance.windowGenerator.NewItemListWindow(changeableRunes, compare: false, added: false, Util.FirstCharToUpper(RuneRef.getRuneTypeName(runeType)), large: false, forceNonEquipment: true, select: true, null, parent as GameObject);
	}
