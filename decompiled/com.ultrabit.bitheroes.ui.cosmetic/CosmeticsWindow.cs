...
public class CosmeticsWindow : WindowsMain
{
	...
	public void LoadDetails(EquipmentRef equipmentRef = null, MountRef mountRef = null, bool isArmory = false)
	{
		...
		if (!_isArmory)
		{
			_cosmeticRef = ((_equipmentRef != null) ? ((ItemRef)GameData.instance.PROJECT.character.equipment.getCosmeticSlot(Equipment.getAvailableSlot(_equipmentRef.equipmentType))) : ((ItemRef)GameData.instance.PROJECT.character.mounts.cosmetic));
		}
		else
		{
			_cosmeticRef = ((_equipmentRef != null) ? ((ItemRef)GameData.instance.PROJECT.character.armory.currentArmoryEquipmentSlot.GetCosmeticSlot(Equipment.getAvailableSlot(_equipmentRef.equipmentType))) : ((ItemRef)GameData.instance.PROJECT.character.armory.currentArmoryEquipmentSlot.GetCosmeticMountEquipped()));
		}
	...
	private void UpdateItems()
	{
		...
		List<MountRef> fullMountList = MountBook.GetFullMountList();
		for (int j = 0; j < fullMountList.Count; j++)
		{
			MountRef mountRef = fullMountList[j];
	...
	private void CreateList()
	{
		...
		cosmeticsList.Data.InsertItemsAtStart(list3);
	...
	private void SaveCosmetic(ItemRef itemRef = null)
	{
		if (_equipmentRef != null)
		{
			if (!_isArmory)
			{
				GameData.instance.PROJECT.character.equipment.setCosmeticSlot(itemRef as EquipmentRef, Equipment.getAvailableSlot(_equipmentRef.equipmentType));
			...
			else
			{
				...
				GameData.instance.PROJECT.character.armory.currentArmoryEquipmentSlot.SetCosmeticSlot(equipRef, Equipment.getAvailableSlot(_equipmentRef.equipmentType));
		...
		if (_mountRef != null)
		{
			if (itemRef != null)
			{
				if (!_isArmory)
				{
					...
					GameData.instance.PROJECT.character.mounts.setCosmetic(mountRef);
					CharacterDALC.instance.doMountCosmetic(mountRef);
				}
				else
				{
					int num = 0;
					...
					GameData.instance.PROJECT.character.armory.currentArmoryEquipmentSlot.mountCosmetic = num;
					CharacterDALC.instance.doSaveArmory(GameData.instance.PROJECT.character.armory.currentArmoryEquipmentSlot);
				}
			}
			else if (!_isArmory)
			{
				GameData.instance.PROJECT.character.mounts.setCosmetic(null);
				CharacterDALC.instance.doMountCosmetic();
			}
			else
			{
