...
public class JSONObject : IEnumerable
{
			...
			else if (str.Length > 0)
			{
				...
				default:
					try
					{
						...
						if (!str.Contains("."))
						{
							i = Convert.ToInt64(str, CultureInfo.InvariantCulture);
							useInt = true;
						}
						type = Type.NUMBER;
						return;
				...
				while (++num2 < str.Length)
				{
					...
					if (text.Length > 0)
					{
						...
						if (maxDepth != -1)
						{
							list.Add(Create(text, (maxDepth < -1) ? (-2) : (maxDepth - 1), storeExcessLevels));
						}
	...
	public static implicit operator WWWForm(JSONObject obj)
	{
		...
		{
			string fieldName = i.ToString(CultureInfo.InvariantCulture);
		...
		if (type == Type.OBJECT)
		{
			...
			for (int i = 0; i < list.Count; i++)
			{
				...
				case Type.STRING:
					...
					break;
				case Type.NUMBER:
					dictionary.Add(keys[i], jSONObject.n.ToString(CultureInfo.InvariantCulture));
					break;
				case Type.BOOL:
					dictionary.Add(keys[i], jSONObject.b.ToString(CultureInfo.InvariantCulture));
					break;
				default:
